...
export const MultiMintButton = ({
    ...
    const [totalCost, setTotalCost] = useState(mintCount * (price + 0));
    ...
    function updateAmounts(qty: number) {
        setMintCount(qty);
        setTotalCost(Math.round(qty * (price + 0.018) * 100000) / 100000);  // 0.012 = approx of account creation fees
    }
    ...
    return (
        <div>
            ...
            {!candyMachine?.state.isSoldOut && !isSoldOut && isActive &&
              <h3>Total estimated cost: {totalCost} SOL</h3>}
        </div>
    );
};
